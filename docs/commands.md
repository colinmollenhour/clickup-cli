...
`cup create` / `cup update` compile descriptions as **ClickUp Flavored Markdown** (CUFM) into native editor blocks, which avoids the extra empty paragraphs ClickUp's `markdown_content` importer inserts around headings. See [ClickUp Flavored Markdown](cufm.md).

## Task sync

Keep a task's description as a local markdown file and push/pull without an LLM in the loop.
...
cup task-sync push notes.md               # upload images, render Mermaid/tldraw, PUT native delta
...
A directory is one graph. Each task is a markdown file. The child's `parent:` is canonical (ClickUp has one parent); the parent's `subtasks:` is order plus a check. Dependencies live in frontmatter too:
...
Images are uploaded as `cup-{sha1}.ext` and reused. Fenced ` ```mermaid ` and ` ```tldraw ` blocks become embedded 2× PNGs plus toggles containing their source. tldraw blocks contain complete `.tldr` JSON and require the external `tldraw` CLI (`npm install --global @kitschpatrol/tldraw-cli`). Synced Content bodies require `CU_SESSION_TOKEN` or `--session-token`; empty `::sync-block{id="…"}` components clone an existing block.

### Session token

ClickUp's public API cannot return a task description as the editor stores it. Three things therefore need the **session JWT** the web app uses: lossless `task-sync pull`, Synced Content updates on push, and doctor's stored-color audit. A personal `pk_` API token does not work for these — ClickUp rejects it (`JWT_008` / `JWT_062`).

Get one from the web app:
...
cup auth session --status         # where the token comes from and when it expires
cup auth session --clear          # forget it
```

The token is stored beside `apiToken` in the profile config (mode 0600) and never printed back. It is short-lived, so expect to repeat this; commands that need it report `expired 20m ago` rather than failing with an opaque HTTP error. Resolution order is `--session-token`, then `CU_SESSION_TOKEN`, then the stored value.

Without a valid token, `task-sync pull` falls back to ClickUp's flattened markdown export, which cannot represent banners, toggles, columns, table widths, diagram sources, or sync-block ids. Pull refuses to overwrite a local file containing CUFM in that case; pass `--lossy` to accept the flattened version. Every pull line ends in `(lossless)` or `(lossy)` so the fidelity is never implicit.
...
Doctor writes every supported text/highlight/badge/block/banner color combination, plus lists, nested toggles, tables, Mermaid, tldraw, columns, quotes, and alerts. Banners include `-strong` variants; other color channels use base colors. It prints a pass/fail report. Keep the task and inspect it in the UI; set `CU_SESSION_TOKEN` to also audit stored Quill color tokens.
...
| `cup auth session [token]`                               | Store the ClickUp session JWT used for lossless pulls and Synced Content |
...
| `cup task-sync init <taskId> [file]`                     | Pull a task (or tree, if dest is a directory) into local CUFM markdown   |
| `cup task-sync push [file]`                              | Push a CUFM file or directory of tasks/subtasks to ClickUp               |
| `cup task-sync pull [file]`                              | Pull a CUFM file or directory of tasks/subtasks from ClickUp             |
| `cup task-sync status [file]`                            | Show local vs remote sync state                                          |
| `cup task-sync doctor`                                   | Create a CUFM torture-test task for visual sanity checks                 |
